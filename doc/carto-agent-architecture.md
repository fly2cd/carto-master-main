...
| MapLibre GL JS + SVG overlay 优先，受控浏览器导出 | 复用现有前端表达链路并保持图面元素可编辑 | 图册、移动端及其他引擎按证据开放 |
...
| `adapters` | ToolGateway、本地/API/MCP 连接器、MapLibre Web 适配器与 Renderer | 受控能力调用、协议转换、隔离工作进程、候选预览与锁后正式渲染 | 自主改变业务语义或冻结参数 |
...
不直接照搬 PPT 的执行自由度。PPT 的 Executor 包含构图设计，不等同于确定性地图渲染器；地图中的投影、分类、符号面积、颜色和位置可能承载业务含义，不能因为属于视觉参数就允许任意改变。地图采用两类业务职责、三个执行阶段：
...
| 计算 | 隔离受控浏览器渲染进程，按资源限并发 | 渲染进程池；需要时增加任务队列 |
| 身份与凭据 | 宿主身份与最小权限凭据 | 企业身份服务、集中凭据管理和审计 |
| 扩展触发 | 单机吞吐、数据容量或多用户需求 | 队列时延、渲染资源、隔离与可用性要求 |

“服务”首先是逻辑接口。知识查询、模板仓库和数据准备不因命名为 Service 就需要独立网络部署。受控浏览器渲染池先独立伸缩；只有明确的规模、团队或安全隔离收益，才拆分内核模块为服务。
...
| B：首个纵向工程样例 | 创建一个洪涝场景模板、真实数据生成、MapPlanner/MapComposer 分工、MapLibre Web 预览与锁后受控浏览器输出 | 交接与修复边界有效，单条链路可恢复可追踪；不宣称三场景已覆盖 |
...
| 9. 正式输出 | Renderer 经受控无头浏览器读取固定 RenderScene/RenderBundle，组合 MapLibre 与 SVG overlay 后输出，不再调用 MapComposer 设计或运行数据子智能体 | attempt 产物、环境与渲染回执、成品检查 |
